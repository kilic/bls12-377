...
func (e *fp6) fromBytes(b []byte) (*fe6, error) {
	...
	u2, err := fp2.fromBytes(b[4*fpByteSize:])
	if err != nil {
		return nil, err
	}
...
func (e *fp6) toBytes(a *fe6) []byte {
	...
	copy(out[:2*fpByteSize], fp2.toBytes(&a[0]))
	copy(out[2*fpByteSize:4*fpByteSize], fp2.toBytes(&a[1]))
	copy(out[4*fpByteSize:], fp2.toBytes(&a[2]))
	return out
}
